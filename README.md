...
Шаблон для создания фасада.

Фасад предназначен для тренсформации HTTP запросов в сообщения брокера сообщений.
...
Шаблон будет установлен под именем ```baal-facade```.
...
```powershell
dotnet new baal-facade --name "{название.сервиса}" --output "путь\к\экземпляру\сервиса" --Description "{описание.сервиса}" --Authors "{авторы}"
```
